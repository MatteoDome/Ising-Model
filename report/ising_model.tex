...
The behaviour of the Binder cumulant is shown in Figure~\ref{bc}. It can be shown that $Q$ has a universal value at the critical point \cite{lecture_notes}. Therefore one can estimate $\beta J_c$ by determining the intersection point of $Q$ for different lattice sizes. STILL MISSING THIS
...
Moreover, the value of the magnetization at $T_c$ is expected to satisfy the following scaling relation \cite{tang}:
\begin{equation}
m(T_c) \propto L^{\beta/\nu}
\end{equation}
