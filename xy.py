...
def simulate(N, T_init, T_end, T_step, n_idle):
    ...
    for i in range(n_iter):
        #   Store x to calculate helicity modulus
        angles[int(i%n_idle), :, :] = np.arctan(lattice[:, :, 1]/lattice[:, :, 0])

        #   Create random vector and normalize
        random_vec = np.random.uniform(-1, 1, size = 2)
        random_vec = random_vec/np.linalg.norm(random_vec)

        #   Take inner product to compute parallel components
        ...
        links = find_links(N, spins, parallel_component, T)
