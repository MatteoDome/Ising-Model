...
with PdfPages('batch_results/magnetization.pdf') as pdf:
    ...
    for N in N_values:
        ...
        plt.plot(magnetization[:,0], magnetization[:,1], label='N = ' + str(N), marker = '.')
...
with PdfPages('batch_results/binder_cumulant.pdf') as pdf:
    ...
    for N in N_values:
        ...
        plt.plot(binder_cumulant[:,0], binder_cumulant[:, 1], label = "N =" + str(N), marker = '.')
        results.close()
...
with PdfPages('batch_results/susceptibility.pdf') as pdf:
    ...
    for N in N_values:
        ...
        plt.plot(susceptibility[:,0], susceptibility[:, 1], label = "N =" + str(N),marker = '.')
...
with PdfPages('batch_results/cv.pdf') as pdf:
    ...
    for N in N_values:
        ...
        cv = cv[cv[:,0].argsort()]
        plt.plot(cv[:,0], cv[:, 1], label = "N =" + str(N),marker = '.')
